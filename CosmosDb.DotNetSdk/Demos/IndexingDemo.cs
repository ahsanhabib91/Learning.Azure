...
namespace CosmosDb.DotNetSdk.Demos
{
	public static class IndexingDemo
	{
		...
		private async static Task ExcludedPaths()
		{
			...
			await Shared.Client.GetDatabase("adventure-works").CreateContainerAsync(containerProps, 400);

			// Load documents 
			var container = Shared.Client.GetContainer("adventure-works", "customindexing");
		...
		private async static Task SpatialIndexes()
		{
			...
			var container = Shared.Client.GetContainer("adventure-works", "spatialindexing");
